...
bool GAMECONTROLLER::can_spawn(PLAYER *player, vec2 *out_pos)
{
	...
	if(is_teamplay())
	{
		...
		//Mole human
 		chance=rand()%100;
 		if(config.dbg_war3 && player->human_mole > 0)dbg_msg("chance","%d %d",chance,player->human_mole*15);
 		if((game.controller)->is_rpg() && player->human_mole && chance <= (player->human_mole*15) && !player->suicide)
...
void GAMECONTROLLER::on_player_info_change(class PLAYER *p)
{
	...
	if(p->race_name != VIDE && (game.controller)->is_rpg())
	{
		switch(p->race_name)
		{
			case ORC:
...
bool GAMECONTROLLER::is_rpg() const
{
	return false;
}
void GAMECONTROLLER::on_level_up(PLAYER *player){}
void GAMECONTROLLER::display_level(PLAYER *player){}
void GAMECONTROLLER::display_stats(PLAYER *player,PLAYER *from){}
